...
internal static class ProgramHelper
{
    ...
    public static void ConfigureHostBuilder<T>(this WebApplicationBuilder builder, string[] args) where T : class
    {
        ...
        builder.Configuration.AddJsonFile("openvisiondata.json", optional: true, reloadOnChange: true);
        var env = builder.Environment;
        builder.Configuration.AddJsonFile($"serilog.{env.EnvironmentName}.json", optional: true, reloadOnChange: true);
        builder.Configuration.AddJsonFile($"openvisiondata.{env.EnvironmentName}.json", optional: true, reloadOnChange: true);
    ...
    public static IHostApplicationBuilder ConfigureOpenVisionServer(this WebApplicationBuilder builder)
    {
        ...
        var openVisionData = builder.Configuration
            .GetSection(nameof(OpenVisionData))
            .Get<OpenVisionData>()!;
        ...
        builder.Services.AddSingleton(openVisionData);
    ...
    public static async Task<bool> ApplyDbMigrationsWithDataSeedAsync(
        IHost host,
        bool applyDbMigrationWithDataSeedFromProgramArguments,
        bool usePooledDbContext,
        ...
        DatabaseMigrationsConfiguration databaseMigrationsConfiguration)
    {
        ...
        using (var serviceScope = host.Services.CreateScope())
        {
            ...
            {
                migrationComplete = await EnsureDatabasesMigratedAsync(services, usePooledDbContext);
            }

            if ((seedConfiguration != null && seedConfiguration.ApplySeed)
                || (applyDbMigrationWithDataSeedFromProgramArguments))
            {
                var seedComplete = await EnsureSeedDataAsync(services);
    ...
    public static async Task<bool> EnsureDatabasesMigratedAsync(this IServiceProvider serviceProvider, bool usePooledDbContext)
    {
        ArgumentNullException.ThrowIfNull(serviceProvider, "app");
        using IServiceScope serviceScope = serviceProvider.CreateScope();
        if (usePooledDbContext)
        {
            using ApplicationDbContext applicationDbContext = serviceScope.ServiceProvider.GetRequiredService<IDbContextFactory<ApplicationDbContext>>().CreateDbContext();
            await applicationDbContext.Database.MigrateAsync();
        }
        else
        {
            using ApplicationDbContext applicationDbContext2 = serviceScope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            await applicationDbContext2.Database.MigrateAsync();
        }

        return true;
    }

    public static async Task<bool> EnsureSeedDataAsync(IServiceProvider serviceProvider)
    {
        using var scope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var openVisionData = scope.ServiceProvider.GetRequiredService<OpenVisionData>();

        await EnsureSeedIdentityServerData(context, openVisionData);
    ...
    /// </summary>
    private static async Task EnsureSeedIdentityServerData(ApplicationDbContext context, OpenVisionData openVisionData)
    {
        foreach (var resource in openVisionData.Databases)
        {
            var exits = await context.Databases.AnyAsync(a => a.Id == resource.Id);
            ...
            await context.Databases.AddAsync(resource);
        }
