...
namespace OpenVision.ML.Maui.Demo.ML.Models.Mobilenet;
...
public class MobilenetVision : VisionSampleBase<MobilenetImageProcessor>
{
    public const string Identifier = "Mobilenet V2";
    public const string ModelFilename = "mobilenetv2-12.onnx";

    public MobilenetVision() : base(Identifier, ModelFilename) { }
    ...
    private List<MobilenetPrediction> GetPredictions(Tensor<float> input)
    {
        ...
        var sum = modelOutput.Sum(x => (float)Math.Exp(x));
        var softmax = modelOutput.Select(x => (float)Math.Exp(x) / sum);
